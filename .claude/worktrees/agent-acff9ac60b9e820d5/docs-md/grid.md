...
The layout toolkit: the centered `wui-container`, a Bootstrap-compatible 12-column `wui-row`/`wui-col` system, the quick `wui-grid` CSS-grid helpers, Tailwind-style `wui-flex` utilities, `wui-gap-*` spacing, and the `wui-bento` dashboard grid. Every demo box is tinted so the layout is legible.
...
`wui-grid-rows-1`, `-3`, `-4`, `-5`, `-6` follow the same pattern (1 through 6 equal-height rows).
...
Full sets: `wui-col-start-1 … 6` and `wui-row-start-1 … 4`.
...
Full sets: `wui-gap-x-0 … 5` and `wui-gap-y-0 … 5`.
...
`is-page` fills its parent's height and never scrolls; `rows-1 … rows-6` choose how many equal-height rows share that space. Requires `min-height:0` up the ancestor chain.
