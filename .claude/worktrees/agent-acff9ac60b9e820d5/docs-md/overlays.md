...
```js
var panel = document.getElementById('api-modal');
var trigger = document.getElementById('api-open-btn');

// Open / close / toggle any overlay by element reference
trigger.addEventListener('click', function () {
  WUI.open(panel, trigger);   // or WUI.toggle(panel, trigger)
});

// Events fire on the panel element
panel.addEventListener('wui:open', function (e) {
  console.log('opened by', e.detail && e.detail.trigger);
});
panel.addEventListener('wui:close', function () {
  console.log('closed');
});

// WUI.anchor positions a floating element near a trigger:
// WUI.anchor(floatingEl, triggerEl, { side: 'bottom', align: 'end', gap: 8, margin: 8 })
```
