...
Structural chrome for a board: the sticky header bar `wui-hdr-wrap`, the inline text link `wui-link`, and the status strip `wui-band-wrap`. Every variant and modifier is shown with its exact markup. For the header's tab strip (`wui-hdr-tabs`), see [Tabs](tabs.md).
