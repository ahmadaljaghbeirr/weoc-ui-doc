...
```js
var cal = WUICalendar.create('#cal-demo', {
  view: 'week',
  events: [
    { id: 1, title: 'EOC Activation Brief',
      start: '2026-06-24T09:00', end: '2026-06-24T10:00',
      variant: 'tier-3', location: 'Command Room A', status: 'Active',
      attendees: ['Aisha Rahman', 'Samir Khan'],
      description: 'Tier-3 standup.' }
    // …more events…
  ],
  actions: [
...
cal.on('action', function (e) {
  if (e.detail.action === 'complete')
    cal.updateEvent(e.detail.event.id, { done: true, variant: 'success' });
});
```
...
| View | Description |
|---|---|
| `month` | 6-week grid, event chips per day, `+N more` rolls into Day view |
| `week` | 7 day columns over an hour timegrid, all-day strip, overlap lanes, now-line |
| `day` | single-column timegrid, same machinery as week |
| `agenda` | chronological list grouped by day for the current month |
...
Drop `data-wui-calendar` on an element and the engine instantiates it on load (and via `WUICalendar.init()` after AJAX/Barba refreshes). Seed events inline with a `data-events` JSON array — ideal for WebEOC views that already render their records to JSON.
