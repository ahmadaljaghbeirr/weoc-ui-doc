...
```js
// weoc-ui.js auto-binds [data-wui-row]; this fallback wires click-to-expand
// for the isolated demo iframe in case the global binder has not run.
document.querySelectorAll('[data-wui-row]').forEach(function (row) {
...
A left-border accent: add one color class to any `<tr>`. On `wui-table-standard` and `wui-table-cards` it draws a 3px left border (cards also inherit the color into the hover borders via `--_row-accent`). On `wui-table-log` the same class drives `--_tier`, so the 4px border and the elapsed-time text color both update.

| Row class | Use case |
|---|---|
| `is-primary` | Active / selected focus |
| `is-success` | Resolved / closed / confirmed |
| `is-info` | Informational / monitoring |
| `is-warning` | Caution / needs attention |
| `is-danger` | Danger / escalated / critical |
| `is-secondary` | Muted / archived / low priority |
