...
> **Reference classes (no inline demo):**
> - `wui-map-panel` — places the map in the `right` grid area of a split shell (`grid-area:right`, full height).
> - `wui-panel-close-btn-map` — `display:none` helper that hides a panel close button in the map context.
> - `map-wrapper` — bordered, rounded, clipped map container; add `is-fs` to break it out to full viewport (`position:fixed; inset:0; z-index:99999`).
> - `location-map-wrap` with `is-fs` — same fullscreen breakout for the location-picker variant.
> - `esri-view` — clears the ESRI SDK's focus outline (`--esri-view-outline-color:unset`); applies only when an ESRI map is mounted.
