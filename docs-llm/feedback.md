...
Components that tell the user what is happening: contextual notices `wui-callout`, page-level `wui-banner`, transient `wui-toast` and `wui-snackbar` (driven by `WUI.toast` / `WUI.snackbar`), the pulsing `wui-alarm`, shimmer placeholders `wui-skeleton`, and empty placeholders `wui-empty-state`. Every variant, size, and state is shown below with its exact markup. For busy overlays and loading states, see the [Loader](loader.md) page.
