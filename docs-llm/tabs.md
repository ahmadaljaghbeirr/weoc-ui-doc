...
The scrollable tab strip `wui-hdr-tabs` that lives inside a board's sticky header. Every variant is shown with its exact markup. For the surrounding header chrome (`wui-hdr-wrap`) and the status strip (`wui-band-wrap`), see the [Navigation](navigation.md) page.
