...
Dashboard tiles use `.wui-tile` > `.wui-tile-hdr` / `.wui-tile-body` / `.wui-tile-footer` (optional header and footer — a body-only tile just omits them). The footer always sits at the bottom regardless of what else is present. Add `.flush` to a header or footer independently to drop its divider border — the same modifier also works on `.wui-panel-hdr`/`-footer`, `.wui-collapse-panel`, and `.wui-card-footer`. Add `.is-scrollable` to `.wui-tile` itself to scroll the body instead of clipping it. See the [KPI & Tile Recipes](charts.md#1-the-tile-shell-wui-tile) section for full copy-paste examples.
