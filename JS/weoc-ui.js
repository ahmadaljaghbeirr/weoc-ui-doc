/* =============================================================================
   ...
   Declarative (attributes, auto-wired)
     ...
     [data-wui-sticky-head="true"]     on a <table>: toggles is-stuck on its <thead> when
                                       the sticky header pins to the top of its scroll
                                       container (CSS swaps the header transparent→opaque).
...
(function (window, document) {
  ...
  /* ═══════════════════════════════════════════════════════════════════════
     ...
     For a table whose transparent sticky <thead> would otherwise let scrolled
     rows bleed through. Opt in with data-wui-sticky-head="true" on the <table>:
     a 0-height sentinel is inserted just before the table and watched with an
     IntersectionObserver; `is-stuck` is toggled on the <thead> the moment the
     header pins to the top of its scroll container, and CSS swaps the header
     from transparent to an opaque surface colour.

     Needs a scrolling ancestor (overflow-y: auto|scroll|overlay — e.g.
     .wui-scroll-area); falls back to the viewport. Idempotent + re-scans after
     WebEOC section refreshes / view switches replace the table.

     WUI.initStickyHeaders([root])   bind any unbound [data-wui-sticky-head] tables
     ═══════════════════════════════════════════════════════════════════════ */
  ...
  WUI.initStickyHeaders = function (root) {
    ...
    var tables = scope.querySelectorAll('[data-wui-sticky-head="true"]');
